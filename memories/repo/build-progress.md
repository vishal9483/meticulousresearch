...
## M5 (in progress) base=main
onboarding: APPROVED after 1 attempt, merged into main (24e79c1 pre-merge), gate=App85/Core434+1skip. Contract=none new. Core.Onboarding: IOnboardingState (persists 'onboarding_completed' in Setting table, Reset), ISampleProjectFactory/SampleProjectFactory (offline sample project + 'Market Research Report' doc artifact), SampleContent. App: OnboardingViewModel wizard, OnboardingCoordinator, IFirstRunHints. SettingsViewModel gained optional IOnboardingState + RerunOnboardingCommand (additive). Downstream v1-acceptance drives first-run.
empty-loading-error-states: APPROVED after 1 attempt, merged into main (ac8b511 pre-merge), gate=App90/Core443+1skip. Contract=view-state pattern: Core.ViewStates (ViewState enum, IUserErrorMapper/UserErrorMapper w/ UserFacingFailureKind + UserFacingException, IErrorLog). App: StatefulViewModel/ListViewModel<T> bases + EmptyState/SkeletonLoader/ErrorState controls (DI IErrorLog->TraceErrorLog, IUserErrorMapper->UserErrorMapper). Downstream accessibility/about-screen + all views consume. NOTE: M1 SQLite pool flake recurred once, green on rerun.
